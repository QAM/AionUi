...
const TelegramConfigForm: React.FC<TelegramConfigFormProps> = ({ pluginStatus, modelSelection, onStatusChange, onTokenChange }) => {
  ...
  useEffect(() => {
    const loadAgentsAndSelection = async () => {
      try {
        const [agentsResp, saved, savedWorkspace] = await Promise.all([acpConversation.getAvailableAgents.invoke(), ConfigStorage.get('assistant.telegram.agent'), ConfigStorage.get('assistant.telegram.workspace')]);
  ...
  const handleWorkspaceChange = async (value: string) => {
    setWorkspace(value);
    try {
      await ConfigStorage.set('assistant.telegram.workspace', value);
